...
var ability_dict = {
    ...
    emhyr_relentless: {
        ...
        activated: async card => {
            ...
            if (window.isReceivingNetworkCards) {
                let expectedData = (window.networkCarouselChoices || []).shift();
                let newCard = window.deserializeCarouselChoice(expectedData, units) || units[0];
                if (newCard) {
                    newCard.holder = card.holder;
                    await board.toHand(newCard, grave);
                }
            ...
            await ui.queueCarousel(grave, 1, (c,i) => {
                ...
                if (window.serializeCarouselChoice && !window.blockNetworkBroadcast && newCard) {
                    window.networkCarouselChoices.push(window.serializeCarouselChoice(newCard, units));
                }
                newCard.holder = card.holder;
    ...
    eredin_bringer_of_death: {
        ...
        activated: async card => {
            ...
            if (card.holder.controller instanceof ControllerAI) {
                ...
                let expectedData = (window.networkCarouselChoices || []).shift();
                newCard = window.deserializeCarouselChoice(expectedData, units) || units[0];
            } else {
                if(Carousel && Carousel.curr) Carousel.curr.exit();
    ...
    eredin_destroyer: {
        ...
        activated: async (card) => {
            ...
            if (window.isReceivingNetworkCards) {
                // Process 2 Discards
                for(let j=0; j<2; j++) {
                    let expDiscard = (window.networkCarouselChoices || []).shift();
                    let dCard = window.deserializeCarouselChoice(expDiscard, hand.cards) || hand.cards[0];
                    if (dCard) await board.toGrave(dCard, hand);
                }
                // Process 1 Draw
                let expDraw = (window.networkCarouselChoices || []).shift();
                let drawCard = window.deserializeCarouselChoice(expDraw, deck.cards) || deck.cards[0];
                if (drawCard) await board.toHand(drawCard, deck);
                return;
            }

            if(Carousel && Carousel.curr) Carousel.curr.exit();
            
            // Local human processing
            await ui.queueCarousel(hand, 2, (c,i) => {
                let dCard = c.cards[i];
                if (window.serializeCarouselChoice && !window.blockNetworkBroadcast && dCard) {
                    window.networkCarouselChoices.push(window.serializeCarouselChoice(dCard, hand.cards));
                }
                board.toGrave(dCard, c);
            }, () => true);
            
            await ui.queueCarousel(deck, 1, (c,i) => {
                let drawCard = c.cards[i];
                if (window.serializeCarouselChoice && !window.blockNetworkBroadcast && drawCard) {
                    window.networkCarouselChoices.push(window.serializeCarouselChoice(drawCard, deck.cards));
                }
                board.toHand(drawCard, deck);
            }, () => true, true);
        },
        weight: (card, ai) => {
    ...
    eredin_king: {
        ...
        activated: async card => {
            let deck = board.getRow(card, "deck", card.holder);
            let weatherCards = deck.findCards(c => c.faction === "weather");
            
            if (card.holder.controller instanceof ControllerAI) {
                await ability_dict["eredin_king"].helper(card).card.autoplay(card.holder.deck);
            } else if (window.isReceivingNetworkCards) {
                let expectedData = (window.networkCarouselChoices || []).shift();
                let chosenCard = window.deserializeCarouselChoice(expectedData, weatherCards) || weatherCards[0];
                if (chosenCard) await board.toWeather(chosenCard, deck);
            } else {
                if(Carousel && Carousel.curr) Carousel.curr.cancel();
                await ui.queueCarousel(deck, 1, (c,i) => {
                    let chosenCard = c.cards[i];
                    if (window.serializeCarouselChoice && !window.blockNetworkBroadcast && chosenCard) {
                        window.networkCarouselChoices.push(window.serializeCarouselChoice(chosenCard, weatherCards));
                    }
                    board.toWeather(chosenCard, deck);
